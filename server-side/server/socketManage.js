const events = require("../../socket-io-client/src/events");
...
module.exports = (io) => (socket) => {
    ...
    socket.on(events.LOGOUT, () => {
        ...
        io.emit(events.LOGOUT, {
            newUsers: users,
            outUser: socket.user.nickname,
        });
    });
    ...
    socket.on(events.P_MESSAGE_SEND, ({ receiver, msg }) => {
        if (socket.user) {
            ...
            socket
                ...
                .emit(events.P_MESSAGE_SEND, { channel: sender, message });
            socket.emit(events.P_MESSAGE_SEND, {
    ...
    socket.on(events.P_TYPING, ({ receiver, isTyping }) => {
        ...
        socket
            .to(receiver)
            .emit(events.P_TYPING, { channel: sender, isTyping });
    });

    socket.on(
        events.CHECK_CHANNEL,
        ({ channelName, channelDescription }, cb) => {
            if (methods.isChannel(channelName, chatsList)) {
                cb(true);
            } else {
                let newChat = methods.createChat({
                    name: channelName,
